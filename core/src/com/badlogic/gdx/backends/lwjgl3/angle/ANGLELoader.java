...
public class ANGLELoader {
	...
	static {
		System.err.println("ENTER OUR ANGLELOADER");
	}
	...
	private static File extractFile (String sourcePath, File outFile) {
		try {
			...
			try {
				...
				in = ArkPets.class.getResourceAsStream("/natives/angle/" + sourcePath);
				byte[] buffer = new byte[4096];
				while (true) {
	...
	public static void load () {
		...
		String crc = crc(ArkPets.class.getResourceAsStream("/natives/angle/" + eglSource))
			+ crc(ArkPets.class.getResourceAsStream("/natives/angle/" + glesSource));
